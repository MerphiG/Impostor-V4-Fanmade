...
function onCreate()
	if getPropertyFromClass('ClientPrefs', 'Optimization') == false then
		makeLuaSprite('floor', 'airship_dark/airFloor', -100, -0);
		setScrollFactor('floor', 1, 1);
		scaleObject('floor', 1, 1);

		makeLuaSprite('cockPit', 'airship_dark/airCockpit', -100, -0);
		setScrollFactor('cockPit', 1, 0.85);
		scaleObject('cockPit', 1, 1);

		makeLuaSprite('equipMent', 'airship_dark/airEquipment', -100, -0);
		setScrollFactor('equipMent', 1, 1);
		scaleObject('equipMent', 1, 1);

		makeLuaSprite('glass', 'airship_dark/airGlass', -100, -0);
		setScrollFactor('glass', 1, 1);
		scaleObject('glass', 1, 0.85);

		makeLuaSprite('map', 'airship_dark/airMap', -100, -0);
		setScrollFactor('map', 1, 1);
		scaleObject('map', 1, 0.85);

		makeLuaSprite('clouds', 'airship_dark/airClouds', -350, -350);
		setScrollFactor('clouds', sf, sf);
		scaleObject('clouds', 1, 1);

		makeLuaSprite('sky', 'airship_dark/airSky', -100, -0);
		setScrollFactor('sky', 1, 1);
		scaleObject('sky', 1, 1);

		makeLuaSprite('light', 'airship_dark/airLight', -100, -0);
		setScrollFactor('light', 1, 1);
		scaleObject('light', 1, 1);
		setProperty('light.alpha', 0.2)

		makeLuaSprite('yellow', 'airship_dark/dead_yellow', -60, 810);
		setScrollFactor('yellow', 1, 1);
		scaleObject('yellow', 1.50, 1.50);

		makeAnimatedLuaSprite( 'dancingf', 'gf_dark', 800,300)
		addAnimationByPrefix('dancingf', 'idle', 'GF FEAR', 24, true);
