...
class ControlsSubState extends MusicBeatSubstate {
	...
	private var grpInputsAlt:Array<AttachedTextW> = [];
	...
	public function new() {
		...
		for (i in 0...optionShit.length) {
			...
			var optionText:AlphabetW = new AlphabetW(0, (10 * i), optionShit[i][0], (!isCentered || isDefaultKey), false);
	...
	private function addBindTexts(optionText:AlphabetW, num:Int) {
		var keys:Array<Dynamic> = ClientPrefs.keyBinds.get(optionShit[num][1]);
		var text1 = new AttachedTextW(InputFormatter.getKeyName(keys[0]), 400, -55);
		text1.setPosition(optionText.x + 400, optionText.y - 55);
		text1.sprTracker = optionText;
		grpInputs.push(text1);
		add(text1);

		var text2 = new AttachedTextW(InputFormatter.getKeyName(keys[1]), 650, -55);
	...
	function reloadKeys() {
		...
		while(grpInputsAlt.length > 0) {
			var item:AttachedTextW = grpInputsAlt[0];
			item.kill();
