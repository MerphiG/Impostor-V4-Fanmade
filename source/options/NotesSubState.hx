...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			for (j in 0...3) {
				var optionText:AlphabetW = new AlphabetW(0, yPos + 60, Std.string(ClientPrefs.arrowHSV[i][j]), true);
		...
		hsbText = new AlphabetW(0, 0, "Hue    Saturation  Brightness", false, false, 0, 0.65);
		hsbText.x = posX + 240;
		add(hsbText);
