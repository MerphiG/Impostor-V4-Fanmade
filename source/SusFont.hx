 ...
 */
class SusFont extends FlxSpriteGroup
{
	...
	var lastSprite:SussyAlphaCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public var isBold:Bool = false;
	public var lettersArray:Array<SussyAlphaCharacter> = [];

	public var finishedText:Bool = false;
	public var typed:Bool = false;

	public var typingSpeed:Float = 0.05;
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, typed:Bool = false, ?textSize:Float = 1, ?typingSpeed:Float = 0.05)
	{
		super(x, y);
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = SussyAlphaCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = SussyAlphaCharacter.symbols.indexOf(character) != -1;
			var isSusFont:Bool = SussyAlphaCharacter.SusFont.indexOf(character.toLowerCase()) != -1;
			if ((isSusFont || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				if (lastSprite != null)
				{
				...
				// var letter:SussyAlphaCharacter = new SussyAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:SussyAlphaCharacter = new SussyAlphaCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = SussyAlphaCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = SussyAlphaCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isSusFont:Bool = SussyAlphaCharacter.SusFont.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isSusFont || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:SussyAlphaCharacter = new SussyAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:SussyAlphaCharacter = new SussyAlphaCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
	...
	override function update(elapsed:Float)
	{
		if (isMenuItem)
		{
			var scaledY = FlxMath.remapToRange(targetY, 0, 1, 0, 1.40);

			var lerpVal:Float = CoolUtil.boundTo(elapsed * 20, 0, 1);
			y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.48) + yAdd, lerpVal);
			if(forceX != Math.NEGATIVE_INFINITY) {
				x = forceX;
			} else {
				x = FlxMath.lerp(x, (targetY * 20) + 150 + xAdd, lerpVal);
			}
		}
...
class SussyAlphaCharacter extends FlxSprite
{
	public static var SusFont:String = "abcdefghijklmnopqrstuvwxyz";
	...
	{
		super(x, y);
		var tex = Paths.getSparrowAtlas('sussyfont');
		frames = tex;
