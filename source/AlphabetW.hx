 ...
 */
class AlphabetW extends FlxSpriteGroup
{
	...
	var lastSprite:AlphaCharacterW;
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if ((isAlphabetW || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterW = new AlphaCharacterW(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterW = new AlphaCharacterW(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			if ((isAlphabetW || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:AlphaCharacterW = new AlphaCharacterW(30 * loopNum, 0, textSize);
				var letter:AlphaCharacterW = new AlphaCharacterW(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
